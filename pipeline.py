...
EVENT_CLUSTER_MAX_GAP = 12      # 同句内动词之间最大允许token距离
EVENT_CLUSTER_SOFT_MAX = 2      # 一个簇最多保留多少个head
...
def _cluster_heads_by_event(heads, sent, max_gap=EVENT_CLUSTER_MAX_GAP, soft_max=EVENT_CLUSTER_SOFT_MAX):
    """
    贪心从左到右聚类, 主体相同或共享地点/目标视为同一事件簇
    额外约束相邻head距离不超过max_gap, 簇内不跨显著时间提示词
    """
    doc = sent.doc
    clusters, cur = [], []
    last = None
...
def _sg_extract_frame_state(frame, rules):
    # location: 取 locative 边 (s, p, o) 中 o
    loc = {}
    for e in (frame.get("edges") or []):
        if not (isinstance(e, (list, tuple)) and len(e) == 3):
            continue
    ...
    # actions: 节点 attributes 中的动态属性（ing/三单等），用于互斥检查
    actions = {}
...
def _sg_audit_consistency(frames, rules, tag="audit"):
    ...
    last_loc = {}     # nid -> loc_id
    prev_edges = []   # 上一帧 edges，辅助判断是否存在移动过渡
